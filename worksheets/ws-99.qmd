...
![](../images/mandantory.png){width=10% height=9% fig-align="right"}
...
Open downloaded OSM file and selct for the following items (main keys "landuse" and "natural")
...
```
Export and project it (EPSG:32632) naming it `vegtypes.gpkg`.


Then select multistrings (key "highway"). 

```
'primary'  'primary_link'  'residential'  'secondary'  'secondary_link'  'tertiary'  'tertiary_link'  'track'
```
Export and project it (EPSG:32632) naming it `surftypes_2.gpkg`.

### polygons vegetation

Open the attribute table of `vegtypes.gpkg` and then the `field calculator`. Note to New Field with 6 alphanumeric charcters named `ENVIMET_ID`. then process the following expressions:
...
Open the attribute table of `vegtypes.gpkg` and the field calculator and process the following expressions:
...
Save the file using `line_surface_1.gpgk`. Now merge `polygons_surface_1.gpkg`and `line_surface_1.gpgk` to `all_surfaces.gpgk`.


Now you can start the `Convert Geodata to Envimet` Plugin.
